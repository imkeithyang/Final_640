...
gamma_binary = -2# rnorm(1,0,1)
delta_t_binary = -1#rnorm(1,0.2,1)
...
ggplot(df_p, aes(x=p_before, color=factor(treatment))) +
...
ggplot(df_avg_p, aes(x = t))+
  geom_line(aes(y=p_untreat)) + 
  geom_point(aes(y=p_untreat)) +
  geom_line(aes(y=p_treat), color="red") + 
  geom_point(aes(y=p_treat), color="black") + 
  geom_line((aes(y=p_treat_para)), linetype="dashed", color="red") +
  geom_point(aes(y=p_treat_para), color="black")
```
...
alpha_poisson = 5# rnorm(1,0,1)
gamma_poisson = 2# rnorm(1,0,1)
delta_t_poissony = -3#rnorm(1,0.2,1)
tau_poisson = -1#rnorm(1,0.2,1)
...
exp(binary_glm$coefficients[5]) - 1
```
...
exp(poisson_glm$coefficients[5]) - 1
```
